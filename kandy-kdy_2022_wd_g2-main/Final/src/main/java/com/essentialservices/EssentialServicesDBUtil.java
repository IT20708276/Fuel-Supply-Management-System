...
public static boolean insertcompany(String cName, String fName, String lName, String type, String reason, String address, String email,float limit,String letter,String password) {
	 ...
	 public static boolean updatecompany(String cid,String cname,String fname,String lname,String sType,String reason,String address,String  email, float fLimit, String letter, String password) {
	    	
	    	try {
	    		
	    		con = DBConnect.getConnection();
	    		stmt = con.createStatement();
	    		String sql = "update essentialservices set companyName='"+cname+"',managerFirstName='"+fname+"',managerLastName ='"+lname+"',serviceType ='"+sType+"',reason='"+reason+"',address ='"+address+"',email ='"+email+"',fuelLimit ='"+fLimit+"',municipalCouncilLetter ='"+letter+"',password ='"+password+"'"
	    				+ "where companyID='"+cid+"'";
	    		int rs = stmt.executeUpdate(sql);
	 ...
	 public static List<CompanyReport> getCompanyRDetails() {
	    	
	    	
	    	ArrayList<CompanyReport> report = new ArrayList<>();
	    	
	    	try {
	    		
	    		con = DBConnect.getConnection();
	    		stmt = con.createStatement();
	    		String sql = "select fuelLimit,count(*) from essentialservices group by fuelLimit;";
	    		rs = stmt.executeQuery(sql);
	    		
	    		
	    		while(rs.next()) {
	    		
	    			float flimit = rs.getFloat(1);
	    			int fcount = rs.getInt(2);
	    			
	    			
	    			
	    			CompanyReport essen = new CompanyReport(fcount,flimit);
	    			report.add(essen);
